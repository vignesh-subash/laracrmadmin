				...
				<div class="nav-tabs-custom">
					...
					<div class="tab-content">
						...
						<div class="tab-pane" id="tab-custom-link">
							...
							{!! Form::open(['action' => '\Vignesh\Laracrm\Controllers\MenuController@store', 'id' => 'menu-custom-form']) !!}
				...
				<div class="dd" id="menu-nestable">
					<ol class="dd-list">
						@foreach ($menus as $menu)
							<?php echo LCAHelper::print_menu_editor($menu); ?>
						@endforeach
					</ol>
				</div>
			</div>
...
<div class="modal fade" id="EditModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['action' => ['\Vignesh\Laracrm\Controllers\MenuController@update', 1], 'id' => 'menu-edit-form']) !!}
...
$(function () {
	...
	$('#menu-nestable').on('change', function() {
		...
		$.ajax({
			url: "{{ url(config('laracrm.adminRoute') . '/lca_menus/update_hierarchy') }}",
			method: 'POST',
			data: {
				jsonData: jsonData,
				"_token": '{{ csrf_token() }}'
	...
	$("#tab-modules .addModuleMenu").on("click", function() {
		...
		$.ajax({
			url: "{{ url(config('laracrm.adminRoute') . '/lca_menus') }}",
			method: 'POST',
			data: {
				type: 'module',
				module_id: module_id,
