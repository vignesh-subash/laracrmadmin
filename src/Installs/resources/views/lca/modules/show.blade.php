...
<div id="page-content" class="profile2">
	...
	@endif
		<div class="col-md-4">
			<div class="row">
				...
				<div class="col-md-9">
					<a class="text-white" href="{{ url(config('laracrm.adminRoute') . '/'.$module->name_db) }}"><h4 data-toggle="tooltip" data-placement="left" title="Open {{ $module->model }} Module" class="name">{{ $module->label }}</h4></a>
					<div class="row stats">
	...
	<ul id="module-tabs" data-toggle="ajax-tab" class="nav nav-tabs profile" role="tablist">
		<li class=""><a href="{{ url(config('laracrm.adminRoute') . '/modules') }}" data-toggle="tooltip" data-placement="right" title="Back to Modules"> <i class="fa fa-chevron-left"></i>&nbsp;</a></li>
	...
	<div class="tab-content">
								...
								<tr>
									...
									<td>
										...
										@if($field['colname'] != $module->view_col)
											<a href="{{ url(config('laracrm.adminRoute') . '/modules/'.$module->id.'/set_view_col/'.$field['colname']) }}" class="btn btn-edit-field btn-success btn-xs" style="display:inline;padding:2px 5px 3px 5px;" id="view_col_{{ $field['colname'] }}"><i class="fa fa-eye"></i></a>
										@endif
									</td>
								</tr>
							@endforeach
		...
		<div role="tabpanel" class="tab-pane fade in p20 bg-white" id="tab-access">
			...
			<form action="{{ url(config('laracrm.adminRoute') . '/save_role_module_permissions/'.$module->id) }}" method="post">
	...
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				{{ Form::open(['route' => [config('laracrm.adminRoute') . '.modules.destroy', 0], 'id' => 'module_del_form', 'method' => 'delete', 'style'=>'display:inline']) }}
...
<div class="modal fade" id="AddFieldModal" role="dialog" aria-labelledby="myModalLabel">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			{!! Form::open(['route' => config('laracrm.adminRoute') . '.module_fields.store', 'id' => 'field-form']) !!}
...
$(function () {
	...
	$('.delete_module').on("click", function () {
		...
		$.ajax({
			url: "{{ url(config('laracrm.adminRoute') . '/get_module_files/') }}/" + module_id,
			type:"POST",
			beforeSend: function() {
	...
	$("#sortable_module_fields").sortable({
		update: function(event, ui) {
			...
			$.ajax({
				url: "{{ url(config('laracrm.adminRoute') . '/save_module_field_sort') }}/"+{{ $module->id }},
	...
	$("#generate_migr").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('laracrm.adminRoute') . '/module_generate_migr') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
	...
	$("#generate_update").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('laracrm.adminRoute') . '/module_generate_update') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
	...
	$("#generate_migr_crud").on("click", function() {
		...
		$fa.addClass("fa-spin");
		$.ajax({
			url: "{{ url(config('laracrm.adminRoute') . '/module_generate_migr_crud') }}/"+{{ $module->id }},
			method: 'GET',
			success: function( data ) {
				$fa.removeClass("fa-refresh");
				$fa.removeClass("fa-spin");
