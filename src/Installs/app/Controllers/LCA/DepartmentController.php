...
class DepartmentsController extends Controller
{
	...
	public function index()
	{
		...
		if(Module::hasAccess($module->id)) {
			return View('lca.departments.index', [
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Departments", "create")) {
			...
			return redirect()->route(config('laracrm.adminRoute') . '.departments.index');
	...
	public function show($id)
	{
		if(Module::hasAccess("Departments", "view")) {
			...
			if(isset($department->id)) {
				...
				return view('lca.departments.show', [
					'module' => $module,
	...
	public function edit($id)
	{
		if(Module::hasAccess("Departments", "edit")) {
			...
			if(isset($department->id)) {
				...
				return view('lca.departments.edit', [
					'module' => $module,
					'view_col' => $this->view_col,
				])->with('department', $department);
			} else {
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laracrm.adminRoute') . '/departments/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Departments", "edit")) {
					$output .= '<a href="'.url(config('laracrm.adminRoute') . '/departments/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}

				if(Module::hasAccess("Departments", "delete")) {
					$output .= Form::open(['route' => [config('laracrm.adminRoute') . '.departments.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
