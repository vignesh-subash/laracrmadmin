...
class OrganizationsController extends Controller
{
	...
	public function index()
	{
		...
		if(Module::hasAccess($module->id)) {
			return View('lca.organizations.index', [
	...
	public function store(Request $request)
	{
		if(Module::hasAccess("Organizations", "create")) {
			...
			return redirect()->route(config('laracrm.adminRoute') . '.organizations.index');
	...
	public function show($id)
	{
		if(Module::hasAccess("Organizations", "view")) {
			...
			if(isset($organization->id)) {
				...
				return view('lca.organizations.show', [
					'module' => $module,
	...
	public function edit($id)
	{
		if(Module::hasAccess("Organizations", "edit")) {
			...
			if(isset($organization->id)) {
				...
				return view('lca.organizations.edit', [
					'module' => $module,
					'view_col' => $this->view_col,
				])->with('organization', $organization);
			} else {
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laracrm.adminRoute') . '/organizations/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
				}
				// else if($col == "author") {
				//    $data->data[$i][$j];
				// }
			...
			if($this->show_action) {
				$output = '';
				if(Module::hasAccess("Organizations", "edit")) {
					$output .= '<a href="'.url(config('laracrm.adminRoute') . '/organizations/'.$data->data[$i][0].'/edit').'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-edit"></i></a>';
				}

				if(Module::hasAccess("Organizations", "delete")) {
					$output .= Form::open(['route' => [config('laracrm.adminRoute') . '.organizations.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
