...
class BackupsController extends Controller
{
	...
	public function create_backup_ajax(Request $request)
	{
		if(Module::hasAccess("Backups", "create")) {
			...
			if(LCAHelper::getLineWithString2($outputStr, "Copying ") == -1) {
				if(LCAHelper::getLineWithString2($outputStr, "mysqldump: No such file or directory") != -1) {
				...
				$dataStr = LCAHelper::getLineWithString2($outputStr, "Copying ");
	...
	public function destroy($id)
	{
		if(Module::hasAccess("Backups", "delete")) {
			...
			return redirect()->route(config('laracrm.adminRoute') . '.backups.index');
		} else {
			return redirect(config('laracrm.adminRoute')."/");
		}
	}
		...
		for($i=0; $i < count($data->data); $i++) {
			for ($j=0; $j < count($this->listing_cols); $j++) {
				...
				if($col == $this->view_col) {
					$data->data[$i][$j] = '<a href="'.url(config('laracrm.adminRoute') . '/backups/'.$data->data[$i][0]).'">'.$data->data[$i][$j].'</a>';
			...
			if($this->show_action) {
				$output = '';
				$output .= '<a href="'.url(config('laracrm.adminRoute') . '/downloadBackup/'.$data->data[$i][0]).'" class="btn btn-warning btn-xs" style="display:inline;padding:2px 5px 3px 5px;"><i class="fa fa-download"></i></a>';

				if(Module::hasAccess("Backups", "delete")) {
					$output .= Form::open(['route' => [config('laracrm.adminRoute') . '.backups.destroy', $data->data[$i][0]], 'method' => 'delete', 'style'=>'display:inline']);
