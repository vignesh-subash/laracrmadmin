...
class LCAHelper
{
	// $names = LCAHelper::generateModuleNames($module_name);
	...
	// LCAHelper::parseValues($field['popup_vals']);
	...
	public static function recurse_copy($src,$dst) {
		...
		while(false !== ( $file = readdir($dir)) ) {
			if (( $file != '.' ) && ( $file != '..' )) {
				if ( is_dir($src . '/' . $file) ) {
					LCAHelper::recurse_copy($src . '/' . $file,$dst . '/' . $file);
				}
				else {
					// ignore files
			...
			foreach ($objects as $object) {
				if ($object != "." && $object != "..") {
					if (is_dir($dir."/".$object))
						LCAHelper::recurse_delete($dir."/".$object);
					else
						unlink($dir."/".$object);
				}
			}
	...
	// LCAHelper::createThumbnail($filepath, $thumbpath, $thumbnail_width, $thumbnail_height);
	...
	// LCAHelper::print_menu_editor($menu)
	public static function print_menu_editor($menu) {
		$editing = \Collective\Html\FormFacade::open(['route' => [config('laracrm.adminRoute').'.lca_menus.destroy', $menu->id], 'method' => 'delete', 'style'=>'display:inline']);
		...
		if(count($childrens) > 0) {
			...
			foreach($childrens as $children) {
				$str .= LCAHelper::print_menu_editor($children);
			}
			$str .= '</ol>';
		}
		$str .= '</li>';
	...
	public static function print_menu($menu, $active = false) {
		...
		$str = '<li'.$treeview.' '.$active_str.'><a href="'.url(config("laracrm.adminRoute") . '/' . $menu->url ) .'"><i class="fa '.$menu->icon.'"></i> <span>'.LCAHelper::real_module_name($menu->name).'</span> '.$subviewSign.'</a>';

		if(count($childrens)) {
			$str .= '<ul class="treeview-menu">';
			foreach($childrens as $children) {
				$str .= LCAHelper::print_menu($children);
			}
			$str .= '</ul>';
		}
		$str .= '</li>';
	...
	public static function print_menu_topnav($menu, $active = false) {
		...
		$str = '<li '.$treeview.''.$active_str.'><a '.$treeview2.' href="'.url(config("laracrm.adminRoute") . '/' . $menu->url ) .'">'.LCAHelper::real_module_name($menu->name).$subviewSign.'</a>';

		if(count($childrens)) {
			$str .= '<ul class="dropdown-menu" role="menu">';
			foreach($childrens as $children) {
				$str .= LCAHelper::print_menu_topnav($children);
			}
			$str .= '</ul>';
		}
		$str .= '</li>';
	...
	public static function setenv($param, $value) {
		...
		$envfile = LCAHelper::openFile('.env');
		$line = LCAHelper::getLineWithString('.env', $param.'=');
