...
class CodeGenerator
{
	...
	**/
	public static function createController($config, $comm = null) {
        ...
        LCAHelper::log("info", "Creating controller...", $comm);
        ...
        file_put_contents(base_path('app/Http/Controllers/LCA/'.$config->controllerName.".php"), $md);
    }
    ...
    public static function createModel($config, $comm = null) {
        ...
        LCAHelper::log("info", "Creating model...", $comm);
    ...
    public static function createViews($config, $comm = null) {
        ...
        LCAHelper::log("info", "Creating views...", $comm);
        // Create Folder
        @mkdir(base_path("resources/views/lca/".$config->dbTableName), 0777, true);
        ...
        file_put_contents(base_path('resources/views/lca/'.$config->dbTableName.'/index.blade.php'), $md);
        ...
        file_put_contents(base_path('resources/views/lca/'.$config->dbTableName.'/edit.blade.php'), $md);
        ...
        file_put_contents(base_path('resources/views/lca/'.$config->dbTableName.'/show.blade.php'), $md);
    }
    ...
    public static function addMenu($config, $comm = null) {
        ...
        LCAHelper::log("info", "Appending Menu...", $comm);
        ...
        // $md = file_get_contents(base_path('resources/views/lca/layouts/partials/sidebar.blade.php'));
        // $md = str_replace("<!-- LCAMenus -->", $menu, $md);
        // file_put_contents(base_path('resources/views/lca/layouts/partials/sidebar.blade.php'), $md);
    }
	...
	**/
	public static function generateMigration($table, $generate = false, $comm = null)
	{
		...
		LCAHelper::log("info", "Model:\t   ".$modelName, $comm);
		LCAHelper::log("info", "Module:\t   ".$moduleName, $comm);
		LCAHelper::log("info", "Table:\t   ".$dbTableName, $comm);
		LCAHelper::log("info", "Migration: ".$migrationName."\n", $comm);
        ...
        if($generate) {
            ...
            if(isset($module)) {
				LCAHelper::log("info", "Module exists :\t   ".$moduleName, $comm);
                ...
                if($fileExists) {
					LCAHelper::log("info", "Replacing old migration file: ".$fileExistName, $comm);
                    $migrationFileName = $fileExistName;
                }
            } else {
				LCAHelper::log("error", "Module ".$moduleName." doesn't exists; Cannot generate !!!", $comm);
            }
        }
        ...
        try {
            LCAHelper::log("line", "Creating migration...", $comm);
        ...
        LCAHelper::log("info", "Migration done: ".$migrationFileName."\n", $comm);
	}
