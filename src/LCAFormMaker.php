...
class LCAFormMaker
{
	...
	public static function input($module, $field_name, $default_val = null, $required2 = null, $class = 'form-control', $params = [])
	{
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "write")) {
			...
			if($unique && !isset($params['unique'])) {
				...
				$params['adminRoute'] = config('laracrm.adminRoute');
				if(isset($row)) {
					$params['isEdit'] = true;
			...
			switch ($field_type->name) {
				...
				case 'Dropdown':
					...
					if($popup_vals != "") {
						$popup_vals = LCAFormMaker::process_values($popup_vals);
					} else {
						$popup_vals = array();
					}
				...
				case 'Taginput':
					...
					$default_val = LCAFormMaker::process_values($default_val);
	...
	**/
	// $values = LCAFormMaker::process_values($data);
	...
	/**
	* Display field using blade directive @lca_display
	**/
		...
		if(Module::hasFieldAccess($module->id, $module->fields[$field_name]['id'], $access_type = "view")) {
			...
			switch ($field_type->name) {
				...
				case 'Decimal':
					...
					break;
				case 'Dropdown':
					$values = LCAFormMaker::process_values($fieldObj['popup_vals']);
					if(starts_with($fieldObj['popup_vals'], "@")) {
						if($value != 0) {
							$moduleVal = Module::getByTable(str_replace("@", "", $fieldObj['popup_vals']));
							if(isset($moduleVal->id)) {
								$value = "<a href='".url(config("laracrm.adminRoute")."/".$moduleVal->name_db."/".$value)."' class='label label-primary'>".$values[$value]."</a> ";
							} else {
						...
						if(starts_with($fieldObj['popup_vals'], "@")) {
							...
							foreach ($values as $key => $val) {
								if(in_array($key, $valueSel)) {
									$valueOut .= "<a href='".url(config("laracrm.adminRoute")."/".$moduleVal->name_db."/".$key)."' class='label label-primary'>".$val."</a> ";
								}
							}
						} else {
							$valueSel = json_decode($value);
	...
	/**
	* Print form using blade directive @lca_form
	**/
	public static function form($module, $fields = [])
	{
		...
		foreach ($fields as $field) {
			$out .= LCAFormMaker::input($module, $field);
		}
		return $out;
	}
	...
	**/
	public static function lca_access($module_id, $access_type = "view", $user_id = 0)
	{
	...
	**/
	public static function lca_field_access($module_id, $field_id, $access_type = "view", $user_id = 0)
	{
